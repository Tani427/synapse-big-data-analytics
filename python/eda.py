"""
Enhanced Exploratory Data Analysis (EDA) – Fixed version.
Handles empty groupby results and corrects filename for Product Demand.
"""
...
def safe_groupby_plot(df, group_col, value_col, plot_type='bar', title="", xlabel="", ylabel="",
    ...
    if grouped.empty:
        log_print(f"⚠️  Groupby result for '{group_col}' by '{value_col}' is empty – skipping plot.")
        return
    fig, ax = plt.subplots()
    if plot_type == 'bar':
...
def analyze_sales(df):
    ...
    if rev_col and df[rev_col].notna().any():
        log_print(f"\nTotal Revenue ({rev_col}): {df[rev_col].sum():,.2f}")
    if cost_col and df[cost_col].notna().any():
        log_print(f"Total Cost ({cost_col}): {df[cost_col].sum():,.2f}")
    if profit_col and df[profit_col].notna().any():
        log_print(f"Total Profit ({profit_col}): {df[profit_col].sum():,.2f}")
    if units_col and df[units_col].notna().any():
        log_print(f"Total Units Sold ({units_col}): {df[units_col].sum():,.0f}")
    ...
    if region_col and rev_col:
        safe_groupby_plot(df, region_col, rev_col,
                          ...
                          xlabel=region_col, ylabel=rev_col,
                          subfolder=subfolder, filename="revenue_by_region.png",
                          color="skyblue")
        if profit_col:
            safe_groupby_plot(df, region_col, profit_col,
                              title=f"Profit by {region_col}",
                              xlabel=region_col, ylabel=profit_col,
                              subfolder=subfolder, filename="profit_by_region.png",
                              color="lightgreen")
    ...
    if item_col and rev_col:
        safe_groupby_plot(df, item_col, rev_col,
                          title=f"Top 10 Revenue by {item_col}",
                          xlabel=item_col, ylabel=rev_col,
                          subfolder=subfolder, filename="revenue_by_item_top10.png",
                          color="coral")

    # Distribution of numeric columns
    for col in df.select_dtypes(include=[np.number]).columns:
        if df[col].notna().sum() == 0:
            log_print(f"⚠️  Column '{col}' has no data – skipping distribution plot.")
            continue
...
def analyze_customers(df):
    ...
    if seg_col and df[seg_col].notna().any():
        safe_groupby_plot(df, seg_col, seg_col, plot_type='bar',
                          ...
                          subfolder=subfolder, filename="segment_distribution.png",
...
# ============================================================
# 3. PRODUCT DEMAND (now using PoductDemand.csv)
# ============================================================
...
def analyze_stores(df):
    ...
    if city_col and df[city_col].notna().any():
        safe_groupby_plot(df, city_col, city_col,
                          title="Top 10 Cities by Number of Stores",
                          xlabel="City", ylabel="Number of Stores",
                          subfolder=subfolder, filename="stores_by_city.png",
                          color="mediumpurple")
...
def analyze_orders(df):
    ...
    if payment_col and rev_col:
        safe_groupby_plot(df, payment_col, rev_col,
                          title=f"Revenue by {payment_col}",
                          xlabel=payment_col, ylabel=rev_col,
                          subfolder=subfolder, filename="revenue_by_payment.png",
                          color="skyblue")

    seg_col = next((c for c in df.columns if "segment" in c.lower()), None)
    if seg_col and rev_col:
        safe_groupby_plot(df, seg_col, rev_col,
                          title=f"Revenue by {seg_col}",
                          xlabel=seg_col, ylabel=rev_col,
                          subfolder=subfolder, filename="revenue_by_segment.png",
                          color="lightgreen")
...
def analyze_shipping(df):
    ...
    if mode_col and df[mode_col].notna().any():
        safe_groupby_plot(df, mode_col, mode_col,
                          ...
                          xlabel="Mode", ylabel="Count",
                          subfolder=subfolder, filename="shipment_mode.png",
    ...
    if rating_col and df[rating_col].notna().any():
        safe_groupby_plot(df, rating_col, rating_col,
                          ...
                          subfolder=subfolder, filename="customer_rating.png",
                          color="gold")
...
def main():
    log_print("🚀 STARTING ENHANCED EXPLORATORY DATA ANALYSIS (FIXED)")
    ...
    datasets = {
        ...
        "products": "PoductDemand.csv",          # corrected filename
        "stores": "Stores.csv",
